...
router.get('/users', authenticate, adminOnly, async (req, res) => {
  try {
    const { rows } = await db.query(
      'SELECT id, username, email, avatar_url, is_verified, is_admin, is_online, created_at FROM users ORDER BY created_at DESC'
    );
    res.json(rows);
  } catch (err) {
...
router.get('/stats', authenticate, adminOnly, async (req, res) => {
  try {
    ...
    const chats = await db.query('SELECT COUNT(*) FROM chats WHERE is_group=false AND is_channel=false');
    const groups = await db.query('SELECT COUNT(*) FROM chats WHERE is_group=true');
    const channels = await db.query('SELECT COUNT(*) FROM chats WHERE is_channel=true');
    const messages = await db.query('SELECT COUNT(*) FROM messages WHERE is_deleted=false');
    res.json({
...
router.post('/channels', authenticate, adminOnly, async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO chats (name, is_channel, description, username, created_by, member_count)
       VALUES ($1, true, $2, $3, $4, 1) RETURNING *`,
