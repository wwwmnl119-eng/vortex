...
const MONGO_URL = process.env.MONGO_URI || process.env.MONGO_URL || "mongodb://127.0.0.1:27017/vortex";
...
const User = mongoose.model("User", new mongoose.Schema({
  ...
  role: {
    type: String,
    enum: ["user", "beta", "moderator", "admin", "delover"],
    default: "user"
  },
...
function hasRole(user, roles) {
  return !!(user && Array.isArray(roles) && roles.map(r => String(r).toLowerCase()).includes(String(user.role || "").toLowerCase()));
}
...
async function ensureSystemChannel() {
  ...
  if (!channel) {
    ...
    if (!channel.pinned) { channel.pinned = true; changed = true; }
...
app.get("/channels", async (_req, res) => {
  const rows = await Channel.find().sort({ pinned: -1, createdAt: 1 });
  res.json(rows);
});
...
app.post("/admin/set-role", async (req, res) => {
  const me = normalizePhone(req.body.me);
  const target = normalizePhone(req.body.target || req.body.phone);
  ...
  if (!["user", "beta", "moderator", "admin", "delover"].includes(role)) {
