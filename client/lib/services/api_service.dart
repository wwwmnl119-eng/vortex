...
class ApiService {
  ...
  // Messages
  Future<List<Message>> getMessages(String chatId, {String? before, String? search}) async {
    String q = '';
    if (before != null) q += '?before=$before';
    if (search != null) q += '${q.isEmpty ? '?' : '&'}search=${Uri.encodeComponent(search)}';
    final data = await get('/messages/$chatId$q') as List;
    return data.map((m) => Message.fromJson(m)).toList();
  }
  Future<void> reactToMessage(String messageId, String emoji) async =>
      await post('/messages/$messageId/react', {'emoji': emoji});
  Future<void> pinMessage(String messageId, bool pinned) async =>
      await post('/messages/$messageId/pin', {'pinned': pinned});
  ...
  Future<User> uploadAvatarBytes(Uint8List bytes, String ext) async {
    return User.fromJson(await post('/users/me/avatar', {
      'base64': 'data:image/$ext;base64,${base64Encode(bytes)}', 'ext': ext,
    }));
  }
