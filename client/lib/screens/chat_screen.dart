...
class _ChatScreenState extends State<ChatScreen> {
    ...
    _socket.on('message_reactions_updated', (data) {
      if (data['chatId'] == widget.chatId && mounted) {
        final reactions = (data['reactions'] as List)
            .map((r) => Reaction.fromJson(Map<String, dynamic>.from(r))).toList();
        setState(() {
          final idx = _messages.indexWhere((m) => m.id == data['messageId']);
  ...
  Future<void> _searchMessages(String q) async {
    ...
    try {
      final msgs = await _api.getMessages(widget.chatId, search: q);
  ...
  void _pinMessage(Message msg) {
    _socket.emitPin(msg.id, widget.chatId, !msg.isPinned);
  }
  ...
  void dispose() {
    ...
    _socket.off('message_reactions_updated');
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: _showSearch
            ...
            : GestureDetector(
                onTap: widget.targetUserId != null
                ...
                child: Row(children: [
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(_typingUser != null ? 'печатает...' : 'нажмите для профиля',
                      style: TextStyle(
                        color: _typingUser != null ? AppColors.blue : colors.textSecondary,
                        fontSize: 11)),
                  ]),
                ]),
              ),
        actions: [
          IconButton(
            icon: Icon(_showSearch ? Icons.close : Icons.search, color: colors.textSecondary),
            onPressed: () {
              setState(() {
                _showSearch = !_showSearch;
                if (!_showSearch) { _searchCtrl.clear(); _loadMessages(); }
              });
            }),
  ...
  Widget _pinnedBar(AppColors colors) {
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(
          _pinned.first.content ?? 'Закреплённое сообщение',
          ...
          children: [
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (msg.messageType == 'image' && msg.fileUrl != null)
                  ...
                  Row(children: [
                    ...
                    Expanded(child: Text(msg.fileName ?? 'Файл',
                      style: TextStyle(color: isMe ? Colors.white : colors.msgInText, fontSize: 14))),
                  ])
                else
                  Text(
                ...
                Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  if (msg.isEdited)
                    Text('  изм.', style: TextStyle(color: isMe ? Colors.white60 : colors.textSecondary, fontSize: 10)),
                  if (isMe) ...[
                    const SizedBox(width: 4),
                    MessageStatusIcon(status: msg.status, size: 14),
                  ],
                ]),
              ]),
            ),
            if (msg.reactions.isNotEmpty)
              Padding(
                ...
                child: Wrap(
                  spacing: 4,
                  children: msg.reactions.map((r) => GestureDetector(
  ...
  Widget _inputBar(AppColors colors) {
    return Container(
      ...
      child: Row(children: [
        IconButton(
          icon: Icon(_showAttach ? Icons.close : Icons.attach_file, color: colors.textSecondary),
